...
library(jmv)
descriptives(data, freq = TRUE)
summary(data)

#masukkan output ke text file
...
# levels(data$sex) <- c("F","M", "variabel lainnya")
...
ggplot(data, aes(hd), xlab) +
  geom_bar(fill = "blue") +
  xlab( "Heart Disease")+
  ylab("Frequency")+
  labs( title = "Heart Disease Frequency", col = "blue")



plot(data$cp, data$hd, xlab = "chest pain", ylab = "Probability of Heart Disease",
     col = "red", pch = 1, cex = 2)
...
logistic <- glm(hd ~ . - age, data=data, family="binomial")
summary(logistic)
...
##or same as:
1-pchisq(409.95 -183.10 , 296-276)
...
#INTERPRETATING MODEL
summary(logistic)
logistic.fit <- glm(hd ~ . - age - chol - fbs - restecg - exang - oldpeak, data = data, family = "binomial" )
summary(logistic.fit)
coef(logistic.fit)
koef <- exp(coef(logistic.fit))
koef

#predict prob for each observation
p2 <- predict(logistic, data, type = "response")
#classify based on probability
c2 <- ifelse(p2 > 0.5, 1, 0)


#create confusion matrix
tab2 <- table(Predicted = c2, Actual = data$hd)
print(tab2)

# #Specificity/Selectivity/True Negative Rate:
#Sp = TN/(TN+FP)
# 
sp <- tab2[1,1]/sum(tab2[ ,1])
print(sp)
# 
# #Sensitivity/Hit Rate/True Positive Rate
#sn = TP/(TP + FN)
# 
sn <- tab2[2,2]/sum(tab2[ ,2])
print(sn)
